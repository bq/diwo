...
var btnConectar = ui.addButton("Connect bluetooth", margin, margin,w,h).onClick(function() {
...
var btnDesconectar = ui.addButton("Disconnect", margin, y,w,h).onClick(function() { // Creamos el botón de conectar
    btClient.disconnect(); 
    btStatus= false;
});
...
var setIntervale = ui.addInput("Set intervale (s)", margin, 5*y, w/2, h).onChange(function(val){ 
    console.log(val);
    tempToSend = val * 1000; // From seconds to miliseconds
});

var setIntervaleBtn = ui.addButton("Set Intervale",margin +w/2,5*y,w/2,h).onClick(function(){
    flag = 1; // set flag to 1 to set intervale
    send(flag,tempToSend);
});
